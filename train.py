...
from models import LSTM, ASCNN, ASGCN,ASGCN_NEW,asgcn_onelayer
...
class Instructor:
    def __init__(self, opt):
        ...
        absa_dataset = ABSADatesetReader(dataset=opt.dataset, embed_dim=opt.embed_dim, use_bert=True,max_len=70)
        
        self.train_data_loader = BucketIterator(data=absa_dataset.train_data, batch_size=opt.batch_size, shuffle=True,max_len=90)
        self.test_data_loader = BucketIterator(data=absa_dataset.test_data, batch_size=opt.batch_size, shuffle=False,max_len=90)
    ...
    def _train(self, criterion, optimizer):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_data_loader):
                ...
                # # ## capsule loss#########################################
                # one_hot = torch.zeros(outputs.shape[0], outputs.shape[1]).scatter_(1, targets.unsqueeze(1).cpu(), 1).to(self.opt.device)
                # self.label_smoothing(one_hot)
                # loss = self.model.capsule_loss(outputs, one_hot)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='asgcn_new', type=str)#astcn，asgcn_onelayer,asgcn_new
    parser.add_argument('--dataset', default='lap14', type=str, help='lap14,twitter, rest14, rest15, rest16，mams')
    ...
    parser.add_argument('--model_name_or_path', default='/extend/ccf_BDCI/bert-base-chinese', type=str)
    ...
    model_classes = {
        ...
        'astcn': ASGCN,
        'asgcn_new':ASGCN_NEW,
        'asgcn_onelayer':asgcn_onelayer,
    }
    input_colses = {
        'lstm': ['text_indices'],
        'ascnn': ['text_indices', 'aspect_indices', 'left_indices'],
        'asgcn': ['text_indices', 'aspect_indices', 'left_indices', 'dependency_graph','concept_mod'],
        'asgcn_new': ['text_indices', 'aspect_indices', 'left_indices', 'dependency_graph'],
        'asgcn_onelayer': ['text_indices', 'aspect_indices', 'left_indices', 'dependency_graph'],
